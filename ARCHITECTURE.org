...
** Versioning
Guile-SSH uses [[https://semver.org/][semantic versioning]].  While the project in the [[https://semver.org/#spec-item-4][0.y.z phase]], =y=
specifies notable changes in API (including the changes that break backward
compatibility) and =z= denotes small backward-compatible changes and bug
fixes.

When the project reach =1.y.z= phase, we must adhere to the recommendations in
the semantic versioning specs.
